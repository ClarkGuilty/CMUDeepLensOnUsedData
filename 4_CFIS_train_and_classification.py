...
model = deeplens_classifier(learning_rate=0.001,  # Initial learning rate
                          ...
                          val_nepoch=12)          # Number of epochs before checking validation scores. 
...
for i in range(4):
    ...
    real_d = Table.read(os.path.join(export_path,'CFIS_real_data_'+str(i)+'.hdf5')) #Data classified with such network (chunk 1).
